...
4. App Serviceの作成画面が表示されます。「**プロジェクトの詳細**」を入力します。
     ...
     ![AZ900_Lab2_04](./media/AZ900_Lab2_04.png)
...
1. App Serviceブレードの画面右にある「**既定のドメイン**」のURLをクリックします。
...
1. App Serviceブレードの左側にある**設定**項目を展開し、「**構成**」を選択して **全般設定** のタブから「**SCM基本認証の発行資格情報**」をオンにします。
    設定の変更を適用するため、必ず **「適用」** をクリックして続行してください。

    ![AZ900_Lab2_11](./media/AZ900_Lab2_11.png)

2. 同じく画面左側のデプロイを展開し、 **「デプロイセンター」** を選択します。

    

3. **「手動デプロイ」** を選択し、ドロップダウンリストから **「外部のGit」** を選択します。

   ![AZ900_Lab2_121](./media/AZ900_Lab2_121.png)

   

4. 外部Gitの設定として以下の内容を入力します。
   入力後は設定の変更を適用するため、必ず **「保存」** をクリックして続行してください。

   | 設定       | 値                                                 |
   | ---------- | -------------------------------------------------- |
   | リポジトリ | **https://github.com/ctct-edu/CTCTAzureQuest.git** |
   | ブランチ   | **main**                                           |

   ![AZ900_Lab2_13](./media/AZ900_Lab2_13.png)

5. **「構成」** のブレードに戻り、 **スタック設定** のタブに切り替えてページ最下部の **スタートアップ コマンド** の項目に以下のコマンドを入力して **適用** をクリックします。
...
1. App Serviceブレードに戻り、画面右にある「**既定のドメイン**」のURLをクリックします。

   ![AZ900_Lab2_16](./media/AZ900_Lab2_16.BMP)

   

2. サンプルのページが表示されます。表示されない場合はコードのクローニング（コピー）とスタートアップに時間を要している可能性があります。少し待って再度アクセスしてください。

![AZ900_Lab2_17](./media/AZ900_Lab2_17.png)
