...
2. ポータルメニューの検索バーに［仮想マシン］と入力して検索結果を選択します。
...
4. 仮想マシンの作成画面が表示されます。
   　
    > 注：一部の環境では、仮想マシン作成画面が新しい **Create-VM エクスペリエンス（プレビュー）** で表示される場合があります。     
    > 　　該当する場合は、画面上部に以下のような案内が表示されます。
    >
    > ![AZ900_Lab1_00](./media/AZ900_Lab1_00.png)
    >
    > プレビュー画面のまま進めると、本ラボ手順書の画面や設定項目と一部異なる場合があります。
    > そのため、画面上部に表示される　**「以前のエクスペリエンスにアクセスするには、ここをクリックします。」**　を選択し、従来の画面へ切り替えてから演習を進めてください。
...
7. 「**インスタンスの詳細**」を入力します。これ以降、指定されていない項目についてはデフォルト値を使用します。
     ...
     > 注：注：##は受講番号です。01 ～ 99の数字が入ります。
     >
     > 　　サイズが見つからない場合は「すべてのサイズを表示」から検索してください。
     >
...
11. 「**次：ネットワーク**」をクリックしてください。

       

12. 「**ネットワークインターフェイス**」の [**仮想ネットワーク**]の項目で「**新規作成**」を選択します。      


13. 「**仮想ネットワークの作成**」で以下の項目を入力し、「**OK**」を押します。既存の項目にあるアドレス範囲や名称をクリックして、指定された値に変更します。
      ...
      | サブネット名                 | **Subnet01**    |
      | （サブネット）アドレス範囲   | **10.#.0.0/24** |

      > 注：##は受講番号です。01 ～ 99の数字が入ります。
      >
      > 注：#も受講番号です。01～09の人は"0"を抜かしてください。1 ～99の数字が入ります。

14. 項番 12 で設定した「仮想ネットワーク」と「サブネット」に変更されたことを確認します。


15. 以降の設定はデフォルトのまま、「**確認および作成**」を選択します。  ​  

16. 「検証に成功しました」と表示されたら、画面左下部の「**作成**」を選択します。

      ![AZ900_Lab1_12](./media/AZ900_Lab1_12.png)

17. 2~5 分ほどで仮想マシンの作成が完了となります。

      ![AZ900_Lab1_13](./media/AZ900_Lab1_13.png)

18. デプロイ完了後、「リソースに移動」を選択してください。

    ![AZ900_Lab1_14](./media/AZ900_Lab1_14.png)

    ​    

19. 作成した仮想マシンブレードが表示されます。 画面内にある「プライマリ NIC パブリック IP」をコピーしてメモ帳に控えてください。 

      > 注：プライマリ NIC パブリック IPはWebページの閲覧時に使用します。
...
2. 「**ネイティブRDP**」の項目の中の「**RDP ファイルのダウンロード**」をクリックしてください。
