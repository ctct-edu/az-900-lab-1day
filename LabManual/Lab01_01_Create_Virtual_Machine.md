...
11. 「**ネットワークインターフェイス**」の [**仮想ネットワーク**]の項目で「**仮想ネットワークの編集**」を選択します。      


12. 「**仮想ネットワークの編集**」で以下の項目を入力し、「**OK**」を押します。サブネットの内容を変更する場合は、既存のサブネット項目右側に表示されている **サブネットの編集（✏鉛筆状のアイコン）** をクリックして名称やアドレス範囲を変更します。
...
13. 項番 12 で設定した「仮想ネットワーク」と「サブネット」に変更されたことを確認します。
...
17. デプロイ完了後、「リソースに移動」を選択してください。

    ![AZ900_Lab1_14](./media/AZ900_Lab1_14.png)

    ​    

18. 作成した仮想マシンブレードが表示されます。 画面内にある「プライマリ NIC パブリック IP」をコピーしてメモ帳に控えてください。 
